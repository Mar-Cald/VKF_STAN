...
simulation <- simulate_data(
  file = "param_recovery/model_file_sim_go_nogo.stan", 
  input_data = dat_mod,
...
# load simulated data
dat_sim <- readRDS("param_recovery/sim_data_1.rds")
...
dat_mod <- list(N_trial = dat_mod$N_trial,
                ...
                subj = dat_mod$subj,
                GO = dat_mod$GO,
                rt = ifelse(dat_mod$GO == 1, dat_sim$rt_pred, 9999),
                minRT = min(dat_sim$rt_pred),
                N_lik = sum(dat_mod$GO == 1),
...
##  run model
recovery <- stan(
  file = "param_recovery/model_file_transf.stan",# Stan program
