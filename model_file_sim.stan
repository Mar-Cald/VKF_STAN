...
generated quantities {
  ...
  for (n in 1:N_subj*N_trial) {
      ...
      //shifted lognormal
      // regression model
      mu = intercept + u_alpha[subj[n]] + predictions[n] * GO[n] * (beta + u_beta[subj[n]]);
      accum1 = lognormal_rng(mu, sigma);
      rt_pred[n] = accum1 + ndt + u_ndt[subj[n]]; // deicision time = rt - non-decision time
      
      // loop across subj
      tn = n < (N_trial * N_subj) ? n : n-1;
      v = subj[n] != subj[tn+1] ? v0 : v; // Initialize volatilities
      w = subj[n] != subj[tn+1] ? omega + u_om[subj[n]] : w;  // Initialize posterior variances
